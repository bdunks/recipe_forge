...
- Read `llm_context\OVERVIEW.md` to understand the projrect
- Find the plan related to the current scope of the review in `llm_context\plan\$ARGUMENTS.md`.  If default, review the full project.
- Only evaluate against requirements appropriate for the current plans's deliverables
...
- READ `llm_context\plan\$ARGUMENTS.md`.
- **IMPORTANT:** Focus on current plans deliverables, not future milestone features
...
- Output the results of your review to the task's **## Output Log** section in a code review file
- Find the code review file in `llm_context\code_review` based on the scope.  If one does not exist, create it.
